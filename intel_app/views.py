    ...
    if request.method == "POST":
        ...
        if send_bundle_response.status_code == 200:
            if data["status"] == "Success":
                ...
                #     f"https://sms.arkesel.com/sms/api?action=send-sms&api_key=SG1iaE9ZSWtWeHdPQmhabGRkTWk5678&to=0{request.user.phone}&from=RijoLink&sms={sms_message}")
                # print(response1.text)
                #
                # response2 = requests.get(
                #     f"https://sms.arkesel.com/sms/api?action=send-sms&api_key=SG1iaE9ZSWtWeHdPQmhabGRkTWk5678&to={phone_number}&from=RijoLink&sms={receiver_message}")
                ...
                if transaction_channel == "ishare":
                    ...
                    if send_bundle_response != "bad response":
                        ...
                        if send_bundle_response["data"]["request_status_code"] == "200" or send_bundle_response["request_message"] == "Successful":
                            ...
                            response1 = requests.get(
                                f"https://sms.arkesel.com/sms/api?action=send-sms&api_key=SG1iaE9ZSWtWeHdPQmhabGRkTWk5678&to=0{user.phone}&from=RijoLink&sms={sms_message}")
                            print(response1.text)
